          ...
          {tasks.length > 0 && (
            <div className="mt-6">
              ...
              <div className="space-y-2 mt-2">
                {tasks.map((task) => {
                  const isTaskExpanded = expandedGroups[`${taskGroup.Name}-task-${task.Name}`] || false;
                  return (
                    <div key={task.Name} className="border dark:border-gray-700 rounded-lg overflow-hidden bg-white dark:bg-gray-800">
                      <div
                        className="p-4 flex justify-between items-center cursor-pointer bg-gray-50 dark:bg-gray-700"
                        onClick={(e) => {
                          e.stopPropagation();
                          onToggleTask(task.Name);
                        }}
                      >
                        <div className="flex items-center">
                          <ExpandIcon isExpanded={isTaskExpanded} />
